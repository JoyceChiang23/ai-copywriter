...
• 避開詞出現：（是否出現使用者指定的「想避開的詞」，若有請具體指出）
...
根據你實際產出的內容誠實評分，每項做到填 ✅，沒做到或有疑慮填 ❌，並給出具體說明，不要預設全部通過：
✅ 或 ❌ 5 組標題均含明確商品品類詞：（說明哪組有、哪組缺）
✅ 或 ❌ 至少 3 組標題含規格詞（尺寸/材質/顏色）：（說明哪組有、哪組缺）
✅ 或 ❌ 5 組標題策略截然不同（非換字換順序）：（說明每組差異，若有兩組太像請標出）
✅ 或 ❌ 無詞語拼接錯誤（如「床沙」「沙床」等）：（若有錯字請標出具體位置）
✅ 或 ❌ 所有標題長度在 30 字以內：（說明最長的一組字數）
✅ 或 ❌ 商品規格正確區分尺寸/顏色/材質/功能：（若有混淆，如把顏色列為材質，請指出）
✅ 或 ❌ CTA 無罐頭句（無「必備」「改善生活」「提升品質」等詞）：（說明實際 CTA 開頭）
✅ 或 ❌ 建議標籤達 12 個以上且均為搜尋詞（無廣告口號如「必備」「神器」）：（說明實際產出數量，若有口號標籤請列出）
✅ 或 ❌ 顏色名稱正確、無「小奶油白」「暖莫蘭迪」等形容詞前綴：（若有請指出）
✅ 或 ❌ 未過度使用「女生」（全文出現超過 2 次視為過度）：（說明出現次數與位置）`;
}
