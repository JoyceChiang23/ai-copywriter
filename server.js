...
輸出以下 JSON 陣列，每個 title 欄位填入真實可用的蝦皮商品標題（不加括號、不加說明文字、直接是標題本身），不加 markdown 代碼標記：
[
...
⚠️ title 欄位必須是真實標題內容，5 個標題方向截然不同，100 字以內
...
下單 CTA：（1-2 句，蝦皮語氣，具體，不誇大，不用「必備」「神器」「改善生活」等詞）
...
根據你實際產出的內容誠實評分，做到填 ✅，沒做到填 ❌，並說明具體原因或哪裡可以改進：
✅ 或 ❌ 5 組標題均含明確商品品類詞：（說明哪組有品類詞、哪組缺少）
✅ 或 ❌ 至少 3 組標題含規格詞（尺寸/材質/顏色）：（說明哪組有、哪組缺）
✅ 或 ❌ 至少 2 組標題含使用情境或目標族群詞：（說明哪組有）
✅ 或 ❌ 5 組標題方向截然不同（不只換字）：（說明各組差異，若雷同請指出）
✅ 或 ❌ 建議標籤數量達 12 個以上：（說明實際產出數量）
✅ 或 ❌ 商品描述開頭是好處而非商品名稱或廣告詞：（說明實際開場）
✅ 或 ❌ 未出現誇大詞或使用者指定避開的詞：（說明有無違規，若有請指出）`;
}
