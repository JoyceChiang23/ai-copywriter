...
12. 商品描述與標籤嚴格禁用以下空泛廣告詞：新生活必備、改善生活品質、享受舒適生活、女生必備、輕鬆擁有、必備神器、生活更加美好、立即選購改善生活、改變你的生活
13. 標題禁止詞語拼接錯誤：不可把兩個詞直接相連造成無意義字串，例如「床沙」「沙床」是錯的，應寫「床邊/沙發邊」「床邊桌、沙發邊桌」等正確組合
14. 標題不可過度使用「女生」：若賣點是輕量好搬，改用「輕量好搬」「一人可移動」「租屋族好搬」等具體詞；不要每組標題都出現「女生必備」「女生專用」
15. 商品規格必須正確分類：尺寸（cm 數）/ 顏色（色彩名稱）/ 材質（桌腳材料）/ 功能（折疊/收納等）分開列，顏色不得歸為材質
16. CTA 禁止以下罐頭句：提升生活品質、新生活必備、女生必備、輕鬆擁有、生活更加美好；CTA 應說明適合場景或行動方式，例如「小空間也能多一張好用桌，選好顏色即可下單」
17. 顏色名稱不可加形容詞前綴，「小奶油白」「暖奶油白」「淡莫蘭迪」都是錯的；顏色就是顏色，直接寫「奶油白」「莫蘭迪」即可
18. 建議標籤必須是搜尋詞或商品分類詞；禁止廣告口號標籤：學生必備、必買好物、生活神器、女生必備、精緻家具；正確範例：折疊邊桌、床邊桌、沙發邊桌、小坪數家具、租屋族家具、60cm邊桌
19. 全部輸出禁止「精緻」「常用」「人群」「廣大」等空泛詞；指族群請用具體詞如「租屋族」「大學生」「上班族」
20. 標題嚴格禁用「必備」「女生專用」「女生款」「男生款」「精緻系」「高質感」等口號型形容詞；替代寫法：用具體族群詞（租屋族/大學生）、具體情境詞（床邊用/沙發旁）、具體規格詞（60cm/可折疊）
21. 全部輸出禁用「提升生活便利性」「提升生活品質」「提升生活質感」「更便利」「更舒適」「更美好」等套語；改用具體場景，例如「床邊、沙發邊、臨時工作都能派上用場」
22. 標題禁止使用驚嘆號（!）和廣告句式（「打造___！」「享受___！」「擁有___！」）；嚴格禁用：必選、完美選擇、最佳選擇、送禮必選、舒適睡眠必備、最完美
23. 不暗示療效或功效保證：不可寫「助眠」「改善睡眠」「舒壓療效」「療癒效果」「改善心情」（除非商品有明確功效認證）；香氛類改用「睡前放鬆」「睡前儀式」「讓空間多一點安靜感」等描述場景感的詞
24. 同一目標族群詞（如「女性上班族」）在整份輸出中最多出現 1 次；用不同角度切入，例如「上班族送禮」「女生禮物」「生日禮物」「交換禮物」`;
...
請依序檢查以下 7 個面向，每項都必須給出具體說明（有問題就指出問題，沒問題也要說明為何沒問題）：

問題列表：
• 關鍵字堆疊：（原標題是否有多個同義詞連排，例如「桌 茶几 懶人桌 邊桌」連在一起）
• 尺寸規格缺失：（是否缺少 cm 數等尺寸詞，讓買家看不出規格）
• 顏色資訊缺失：（是否缺少顏色選項，是否需要加入標題）
• 適用族群缺失：（是否缺少目標族群詞，如「租屋族」「學生」）
• 使用情境缺失：（是否缺少場景詞，如「床邊」「沙發邊」「筆電桌」）
• 避開詞出現：（⚠️ 必須逐一對照以下避開詞清單，直接比對原始標題每個字：避開詞＝「${avoidWords || '（使用者未填寫）'}」；只要原始標題「${originalTitle}」中出現其中任何一個詞，必須寫「有，包含「XXX」」，絕對不能誤判為「無」）
• 差異化賣點：（買家 3 秒內是否能看懂這個商品的差異化，若看不出來請說明原因）
...
各組策略（5 組方向必須截然不同，不只換字）：
- 高搜尋曝光版：品類詞＋規格詞＋常見搜尋詞，例如「大豆蠟香氛蠟燭 180g｜木芯 白茶雪松 居家香氛 生日禮物」
- 高點擊吸引版：從情境或使用場景切入，不直接說商品名稱，例如「下班後的房間香氣｜白茶雪松木芯香氛蠟燭 180g」
- 高轉換銷售版：使用情境＋解決的需求＋規格，例如「睡前放鬆用香氛蠟燭｜大豆蠟木芯 白茶雪松 送禮可用」
- 簡潔質感版：20 字以內，主要規格詞，乾淨不堆疊，例如「白茶雪松木芯香氛蠟燭｜180g 大豆蠟」
- 長尾關鍵字版：族群詞＋情境詞＋品類詞＋規格，例如「女生生日禮物 香氛蠟燭｜房間香氛 大豆蠟木芯 白茶雪松」

⚠️ 標題格式要求：
- 禁止驚嘆號（!）
- 禁止廣告句式：「打造___！」「享受___！」「必選！」「完美選擇」「最佳選擇」「送禮必選」
- 不暗示療效（助眠/改善睡眠/舒壓效果），只描述使用場景
- 每組 title 必須是能直接貼上蝦皮的商品標題，字數控制在 40 字以內

輸出以下 JSON 陣列，title 填入真實蝦皮商品標題（不加括號或說明文字），不加 markdown 代碼標記：
[
{"type":"高搜尋曝光版","title":"實際蝦皮標題","purpose":"新品上架，追求搜尋曝光","keywordsUsed":["品類詞","規格詞","搜尋熱詞"],"reason":"優勢說明"},
{"type":"高點擊吸引版","title":"實際蝦皮標題","purpose":"有曝光後追求點擊率","keywordsUsed":["詞1","詞2"],"reason":"優勢說明"},
{"type":"高轉換銷售版","title":"實際蝦皮標題","purpose":"有流量時提升轉換率","keywordsUsed":["詞1","詞2"],"reason":"優勢說明"},
{"type":"簡潔質感版","title":"實際蝦皮標題","purpose":"走質感路線的賣家","keywordsUsed":["詞1","詞2"],"reason":"優勢說明"},
{"type":"長尾關鍵字版","title":"實際蝦皮標題","purpose":"鎖定細分族群","keywordsUsed":["詞1","詞2"],"reason":"優勢說明"}
]
⚠️ title 必須是真實標題，100 字以內；5 組 title 不能是同一組關鍵字換順序
...
開場三行賣點（開頭說買家最在意的好處，不說商品名稱，不用空泛廣告詞）：
• （具體好處一，例如解決什麼問題）
• （具體好處二）
• （具體好處三）
...
使用情境：（2-3 句具體場景，描述買家實際怎麼用）

適合對象：（具體說明哪類買家適合）

商品規格（根據商品類型自動選用合適欄位名稱，不要硬套固定欄位）：
判斷邏輯：
• 家具/居家用品 → 尺寸（cm）、顏色、材質、功能
• 香氛/蠟燭/擴香 → 容量或重量（g/ml）、香調（香味名稱）、燭芯（若有）、材質
• 保養品/清潔品 → 容量（ml/g）、成分/香調、適用膚質
• 食品/飲品 → 容量或重量（g/ml）、口味、包裝
• 服飾/配件 → 尺寸、顏色、材質
只列出適用欄位，不套用不相關的欄位；重量（如 180g）→ 容量/重量，香味（如白茶/雪松）→ 香調，不可把這兩者誤歸類成尺寸或顏色

下單 CTA（1-2 句，蝦皮語氣，具體說明適合場景或如何選購，禁止「必備」「神器」「改善生活」「提升品質」等詞，例如：「小空間也能多一張好用桌，選好顏色即可下單。」）：

【模組四：建議標籤】

（12-20 個，逗號分隔，每次至少輸出 12 個）
原則：
- 必須是搜尋詞或商品分類詞，例如：折疊邊桌、床邊桌、沙發邊桌、小坪數家具、租屋族家具、60cm邊桌
- 禁止廣告口號：學生必備、必買好物、生活神器、女生必備、精緻家具、輕鬆生活
- 不重複關鍵字分類裡已有的詞，以補充搜尋詞為主
...
根據這次的商品資訊與使用者填寫的避開詞，條列 4-7 點說明你在這次優化中具體做了哪些改進。

格式規則：
- 每點以「已...」開頭，說明具體動作與結果（例如「已移除」「已補上」「已加入」「已拆分」）
- 只寫有正向動作的項目，不寫後台品質檢查語（禁止寫「未過度使用女生」「無拼接錯誤」「CTA 無罐頭句」「顏色名稱正確」等）
- 若使用者有填寫避開詞，必須明確列出是哪些詞（例如「已避免使用『療癒神器』『必買』『網紅爆款』等使用者指定避開詞」）
- 若有原始標題，必須提及相較原始標題做了哪些具體改進
- 根據本次實際商品內容撰寫，不要照抄範例
- 禁止空話：不可寫「已針對不同方向進行獨特設計」「已突顯商品特點」「已優化各面向」「已充分呈現商品價值」；每點必須包含本次商品的實際詞彙（如具體規格、香調名稱、用途詞）
- 若使用者提供了模糊描述（如「香味舒服」），必須說明已轉換成哪些具體詞彙（如「白茶、雪松」）
- 若輸出整理了規格分類，必須說明用了哪些欄位名稱（如「容量/重量、香調、燭芯、材質」）

以下為格式範例（根據實際商品內容調整）：
1. 已移除原標題中的「懶人桌」，符合使用者避開詞要求
2. 已補上尺寸資訊，讓買家快速看懂規格
3. 已加入適用族群：租屋族、小套房住戶、學生、上班族
4. 已補上使用情境：床邊、沙發邊、筆電桌、早餐桌
5. 已將標題拆成 5 種不同方向：搜尋曝光、點擊吸引、轉換銷售、簡潔質感、長尾關鍵字
6. 商品描述已整理成開場賣點、商品特色、使用情境、適合對象、商品規格與下單 CTA`;
}
